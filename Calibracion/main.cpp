...
void cleanCircles(vector<Vec3f> circles);
...
//Video
  ...
  CvCapture* cap = cvCaptureFromAVI("/home/josue/Devel/videos/rings1.avi");
...
Mat process (Mat image)
{
    ...
    HoughCircles(result, circles, HOUGH_GRADIENT, 1,
                 1.2,  // change this value to detect circles with different distances to each other
                 100, 30, 1, 30 // change the last two parameters
            // (min_radius & max_radius) to detect larger circles
    );
    cleanCircles(circles);
    for( size_t i = 0; i < circles.size(); i++ )
    {
        Vec3i c = circles[i];
        Point center = Point(c[0], c[1]);
...
void cleanCircles(vector<Vec3f> circles) {
    // printf("muajajajajaj");
    printf("%zu-", circles.size());
    // for (int i = 0; i < circles)
}
